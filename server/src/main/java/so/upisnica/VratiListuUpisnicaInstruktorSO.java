 ...
 * @author Tijana
 */
public class VratiListuUpisnicaInstruktorSO extends OpstaSO {
    ...
    protected void izvrsi(Object parametar, Object uslov) throws Exception {
        ...
        if (imePrezime.length == 2) {
            upit1 += " WHERE (instruktor.ime='" + imePrezime[0] + "' AND instruktor.prezime='" + imePrezime[1] + "') OR (instruktor.ime='" + imePrezime[1] + "' AND instruktor.prezime='" + imePrezime[0] + "') ";
        } else {
            upit1 += " WHERE instruktor.ime='" + imePrezime[0] + "' OR instruktor.prezime='" + imePrezime[0] + "'";
        }
